(function (global) {
  const { syncAppLinks, applyFavicon, bootWhenDocumentReady } = global.RAFOTimeApp;
  ...
  class PrecisionClock {
    ...
    async init() {
      ...
      this.displayManager.normalizeLegacyModeQuery();
    ...
    startRenderLoop() {
      ...
      const renderFrame = () => {
        if (!this.displayManager.paused) {
          ...
          this.displayManager.updateDigital(now);
        }
